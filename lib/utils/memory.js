...
Return a JSON object with these 5 keys (goals, challenges, wins, preferences, context). Keep each insight concise but specific. If no clear insight for a category, use an empty string.`;
    ...
    // Save to coaching_memories table
    const { data, error } = await supabase
      ...
      .insert({
        ...
        context: `${toolType.toUpperCase()}: ${insights.context || ''}`,
    ...
    console.log('[MEMORY] Tool memory summary created:', { id: data.id, toolType });
    return true;

  } catch (error) {
    console.error('[MEMORY] Error creating tool memory summary:', error);
    return false;
  }
}
...
export async function getCoachingContext(userId) {
  try {
    ...
    const { data: memories, error } = await supabase
      .from('coaching_memories')
      .select('goals, challenges, wins, preferences, context, created_at')
      .eq('user_id', userId)
    ...
    // Combine insights, giving more weight to recent memories
    ...
    memories.forEach((memory, index) => {
      ...
      if (memory.goals) combinedInsights.goals.push(`${memory.goals} (${weight})`);
      if (memory.challenges) combinedInsights.challenges.push(`${memory.challenges} (${weight})`);
      if (memory.wins) combinedInsights.wins.push(`${memory.wins} (${weight})`);
      if (memory.preferences) combinedInsights.preferences.push(`${memory.preferences} (${weight})`);
      if (memory.context) combinedInsights.context.push(`${memory.context} (${weight})`);
    });
