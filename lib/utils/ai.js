...
export async function getAIResponse(userInput, thread) {
  ...
  console.log('[AI Service] ⚠️ DEBUGGING LOADING STATES - AI REQUEST STARTING ⚠️');
...
Guidance: Ask the user the following question next, based on their previous answers: "${nextQuestion.question}"`;
    ...
    console.log('[AI Service] ⚠️ SENDING API REQUEST - LOADING SHOULD BE TRUE ⚠️');
    
    // For regular chat (no tool), handle streaming response
    if (!tool) {
      console.log('[AI Service] Regular chat - using streaming response');
      try {
        ...
        if (!response.ok) {
          ...
          console.error(`[AI Service] Streaming API call failed with status ${response.status}:`, errorText);
        ...
        console.log('[AI Service] Streaming completed. Final length:', accumulatedResponse.length);
        console.log('[AI Service] ⚠️ STREAMING COMPLETE - LOADING WILL BE SET TO FALSE ⚠️');
        ...
        console.error('[AI Service] Error in streaming response:', error);
    ...
    console.log('[AI Service] ⚠️ API RESPONSE RECEIVED - LOADING SHOULD STILL BE TRUE ⚠️');
    
    // Add a small delay before returning so loading state is more visible
    await new Promise(resolve => setTimeout(resolve, 500));
    
    console.log('[AI Service] ⚠️ RETURNING RESPONSE - LOADING WILL BE SET TO FALSE ⚠️');
