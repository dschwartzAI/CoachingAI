...
// Helper function to build profile context string
// Can accept either userId (will fetch profile) or profile object directly
export async function buildProfileContext(userIdOrProfile) {
  if (!userIdOrProfile) return '';
  
  let profile;
  
  // If it's a string, treat it as userId and fetch profile
  if (typeof userIdOrProfile === 'string') {
    try {
      profile = await getUserProfile(userIdOrProfile);
      if (!profile) return '';
    } catch (error) {
      if (process.env.NODE_ENV !== "production") console.error('[AI Service] Error fetching user profile for context:', error);
      return '';
    }
  } else {
    // It's already a profile object
    profile = userIdOrProfile;
  }
  ...
  if (profile.occupation) contextParts.push(`Business: ${profile.occupation}`);
  if (profile.current_mrr && profile.desired_mrr) {
    contextParts.push(`Revenue: Currently ${profile.current_mrr}, targeting ${profile.desired_mrr}`);
  } else if (profile.current_mrr) {
    contextParts.push(`Current Revenue: ${profile.current_mrr}`);
  } else if (profile.desired_mrr) {
    contextParts.push(`Target Revenue: ${profile.desired_mrr}`);
  }
  if (profile.desired_hours) contextParts.push(`Desired Work Hours: ${profile.desired_hours} per week`);
  if (profile.business_stage) contextParts.push(`Business Stage: ${profile.business_stage}`);
  if (profile.biggest_challenge) contextParts.push(`Biggest Challenge: ${profile.biggest_challenge}`);
  if (profile.primary_goal) contextParts.push(`Primary Goal: ${profile.primary_goal}`);
  
  // Add psychographic brief if available - this is extremely valuable context for copy generation
  if (profile.psychographic_brief) {
    contextParts.push(`\n=== IDEAL CLIENT PSYCHOGRAPHIC BRIEF ===\n${profile.psychographic_brief}\n=== END PSYCHOGRAPHIC BRIEF ===`);
  }
  
  if (contextParts.length === 0) return '';
  
  return `\n\n=== USER CONTEXT ===\n${contextParts.join('\n')}\n\nPlease use this context to personalize your responses and make them more relevant to their specific business situation and current challenges.\n=== END USER CONTEXT ===\n\n`;
}

export async function getAIResponse(userInput, thread, userId = null) {
  ...
  // Get user profile for context and tool logic
  let userProfile = null;
  if (userId) {
    try {
      userProfile = await getUserProfile(userId);
    } catch (error) {
      if (process.env.NODE_ENV !== "production") console.error('[AI Service] Error fetching user profile:', error);
    }
  }

  // Build profile context that will be added to all system messages
  const profileContext = await buildProfileContext(userProfile || userId);

  // Prepare variables for tool question logic so they can be used later when
  ...
  try {
    ...
    if (tool) {
      ...
      // Inject profile context into tool system message
      systemMessageContent += profileContext;
      ...
      ({ nextQuestion, allQuestionsAnswered, answers } = getNextQuestion(tool, allMessages, userProfile));
...
Guidance: Ask the user the following question next, based on their previous answers: "${nextQuestion.question}"`;
      ...
      // --- Standard Chat Logic ---
      // Inject profile context into regular chat system message
      systemMessageContent += profileContext;
      
      if (process.env.NODE_ENV !== "production") console.log('[AI Service] Standard Chat Flow with profile context');
      messagesToSend.push({ role: 'system', content: systemMessageContent });
    ...
    const requestBody = {
      ...
      // Pass userId for profile context in API
      userId: userId
    };
