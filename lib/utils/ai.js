...
export async function buildProfileContext(userIdOrProfile) {
  ...
  if (profile.desired_mrr) contextParts.push(`Target Monthly Revenue: ${profile.desired_mrr}`);
  if (profile.desired_hours) contextParts.push(`Desired Work Hours: ${profile.desired_hours} per week`);
  
  if (contextParts.length === 0) return '';
  
  return `\n\n=== USER CONTEXT ===\n${contextParts.join('\n')}\n\nPlease use this context to personalize your responses and make them more relevant to their specific business situation.\n=== END USER CONTEXT ===\n\n`;
}
