...
export async function saveMessage(message, userId) {
  if (!userId) {
    console.error('[Supabase] No user ID provided for saveMessage');
...
export async function deleteThread(threadId) {
  console.log('[Supabase] Deleting thread:', threadId);
  
  if (!threadId) {
    console.error('[Supabase] Cannot delete thread: No thread ID provided');
  ...
  try {
    ...
    if (checkError) {
      console.error('[Supabase] Error checking thread existence:', checkError);
      
      if (checkError.code === 'PGRST116') {
        console.error('[Supabase] Thread not found with ID:', threadId);
    ...
    console.log('[Supabase] Found thread to delete:', existingThread);
    
    // Get current user session to verify permissions
    const { data: { session } } = await supabase.auth.getSession();
    if (!session) {
      console.error('[Supabase] No active session when attempting to delete thread');
    } else {
      console.log('[Supabase] Active session user ID:', session.user.id);
      console.log('[Supabase] Thread user ID:', existingThread.user_id);
      console.log('[Supabase] User authorized to delete?', session.user.id === existingThread.user_id);
    }
    ...
    if (error) {
      console.error('[Supabase] Error deleting thread:', error);
      
      // Handle RLS errors specially
      if (error.code === '42501') {
        console.error('[Supabase] Row Level Security prevented thread deletion');
        console.error('[Supabase] This is likely a permissions issue - the current user may not be allowed to delete this thread');
    ...
    console.log('[Supabase] Thread deleted successfully:', threadId);
    return true;
  } catch (error) {
...
export { getThreads as fetchThreads }; 