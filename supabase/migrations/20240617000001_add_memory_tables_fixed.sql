-- Enable vector extension and add memory tables (fixed version)
CREATE EXTENSION IF NOT EXISTS vector;

-- Enum type for memory categories
CREATE TYPE memory_type AS ENUM ('episodic','fact','preference','artefact');

-- Table storing individual user memories (with text thread_id to match threads table)
...
CREATE POLICY "Allow dev users to select user_memories"
  ON user_memories
  FOR SELECT
  USING (user_id::text LIKE 'dev-user-%');

CREATE POLICY "Allow dev users to insert user_memories"
  ON user_memories
  FOR INSERT
  WITH CHECK (user_id::text LIKE 'dev-user-%');
