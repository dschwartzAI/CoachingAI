...
export function useChatSSE({ user, currentChat, setCurrentChat, chats, setChats, scrollToBottom, textareaRef }) {
  ...
  // Ensure any active SSE connection is closed on unmount
  useEffect(() => {
    return () => {
  ...
  const closeConnection = () => {
    if (eventSourceRef.current) {
      console.log('[SSE Connect] Closing EventSource connection');
      eventSourceRef.current.close();
      eventSourceRef.current = null;
    }
  };
  ...
  return { connectToN8nResultStream, closeConnection, isWaitingForN8n, setIsWaitingForN8n, eventSourceRef };
}
