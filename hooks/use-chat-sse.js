...
export function useChatSSE({ user, currentChat, setCurrentChat, chats, setChats, scrollToBottom, textareaRef }) {
  ...
  // Ensure any open connection is closed when the component using this hook unmounts
  useEffect(() => {
    return () => {
      closeConnection();
    };
  ...
  return { connectToN8nResultStream, isWaitingForN8n, setIsWaitingForN8n, eventSourceRef, closeConnection };
}
