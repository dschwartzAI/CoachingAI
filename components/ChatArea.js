...
export default function ChatArea({ selectedTool, currentChat, setCurrentChat, chats, setChats }) {
  ...
  const { connectToN8nResultStream, closeConnection, isWaitingForN8n, setIsWaitingForN8n, eventSourceRef } =
    useChatSSE({ user, currentChat, setCurrentChat, chats, setChats, scrollToBottom, textareaRef });

  // Clean up SSE connection when ChatArea unmounts
  useEffect(() => {
    return () => {
      closeConnection();
    };
